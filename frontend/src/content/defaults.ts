/**
 * The site's copy as it ships in the bundle.
 *
 * This is what renders when the content API is unreachable — a portfolio that
 * still works with the backend down. It is also what the backend was seeded
 * from (`backend/app/seed.py`), so the two start identical and drift only as
 * the CMS is used.
 *
 * Edit content in the CMS, not here. Refresh these literals only when you want
 * the offline copy to catch up.
 */
...
export const experience: ExperienceContent = {
  ...
  footnotes: [
    { text: 'License in Computer Science · Al Maaref University · 2019–2023' },
    { text: 'Arabic (native) · English (professional) · French (basic)' },
  ],
};
